...
import { RecycleChain, RecycleChain__factory } from "../../../../standalone/recylce-chain-contract/typechain-types";
...
export class ListenerService implements OnModuleInit, OnModuleDestroy {
    ...
    subscribeToEvents() {
        try {
            this.contract.on(this.contract.filters.ManufacturerRegistered, async (
                manufacturer,
                name,
                location,
                contact,
                ...
                await this.createManufacturer({
                    contact, id: manufacturer, location, name, timestamp
                });
        ...
        try {
            this.contract.on(this.contract.filters.ProductCreated, async (
                productId, name, manufacturerId, event
            ) => {
                // @ts-expect-error //the block number does not exist inside the event.blockNumber
                const blockNumber = event.log.blockNumber;
                const timestamp = await this.getBlockTimeStamp(blockNumber);
        ...
        try {
            this.contract.on(this.contract.filters.ProductItemAdded, async (
                productItemIds, productId, event
            ) => {
                // @ts-expect-error //the block number does not exist inside the event.blockNumber
                const blockNumber = event.log.blockNumber;
                const timestamp = await this.getBlockTimeStamp(blockNumber);
            ...
            console.log('Event: ProductItemsAdded Listening: Listener setup failed.', error);
        }

        try {
            this.contract.on(
                this.contract.filters.ProductItemsStatusChanged, async (
                    productItemIds,
                    statusIndex,
                    event
                ) => {
            ...
            console.log('Event: ProductItemStatusChanged: Listener setup failed', error);
        }

        try {
            this.contract.on(
                this.contract.filters.ToxicItemCreated, async (
                    productId,
                    name,
                    weight,
                    event
